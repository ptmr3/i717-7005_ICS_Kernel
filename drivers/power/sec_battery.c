...
#if !defined(CONFIG_HKTW_MODEL_GT_N7005)
...
static void sec_bat_update_info(struct sec_bat_info *info)
{
	...
	pr_info("%s : chg_status : %d,health: %d,presoc: %d, temp: %d,cable_type: %d, vbatt: %d,soc: %d,full_soc: %d,current: %d,ui_full: %d\n",
		__func__, info->charging_status,info->batt_health,info->batt_presoc,info->batt_temp,info->cable_type,info->batt_vcell,info->batt_soc,\
		info->batt_full_soc,info->batt_current_adc,info->ui_full_charge_status);
}
