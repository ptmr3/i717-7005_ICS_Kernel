...
//kk0704.park :: INTEGRATION FROM ICS_MAIN
int detached_status;
...
void fsa9480_manual_switching(int path)
{
	...
	if (path ==  SWITCH_PORT_ALL_OPEN) {
		...
		check_again = i2c_smbus_read_byte_data(client, FSA9480_REG_CTRL);
		if(check_again >= 0){
			if(data != check_again){
				printk(KERN_DEBUG "%s: not same data\n", __func__);
				ret = i2c_smbus_write_byte_data(client, FSA9480_REG_MANSW1, data);
				if (ret < 0)
					dev_err(&client->dev, "%s: err %d\n", __func__, ret);
			}
...
static void fsa9480_detect_dev(struct fsa9480_usbsw *usbsw)
{
	...
	if (val1 || val2) {
		...
		if (val1 & DEV_T1_USB_MASK || val2 & DEV_T2_USB_MASK) {
			...
			dev_info(&client->dev, "usbsw->mansw: 0x%x\n", usbsw->mansw);			
			dev_info(&client->dev, "local_usbsw->mansw: 0x%x\n", local_usbsw->mansw);						
			if (pdata->usb_cb)
