				...
				const char *buf, size_t size)
{
	...
	if (err < 0)
		return err;
	pr_err("[TAOS] %s: new delay = %lldns, old delay = %lldns\n",
...
static ssize_t proximity_avg_store(struct device *dev, struct device_attribute *attr, const char *buf, size_t size)
{
	...
	if (new_value == true) {
		...
		pr_err("%s: timer start, delay %lldns\n",
			__func__, ktime_to_ns(prox_polling_time));

	} else {
		hrtimer_cancel(&taos->ptimer);
		cancel_work_sync(&taos->work_ptime);
		pr_err("%s: cancelling prox_avg timer\n", __func__);
	}
...
static void taos_work_func_prox(struct work_struct *work) 
{
	...
	if (taos_get_lux() >= 1500) {
		...
		pr_err("%s: Error! Not Common Case! adc(%d), th_high(%d), th_low(%d)\n",
			__func__, adc_data, threshold_high, threshold_low);
	}

	input_report_abs(taos->proximity_input_dev, ABS_DISTANCE, !proximity_value);
	pr_err("%s: %d (0: close, 1: far) \n", __func__, !proximity_value);
	input_sync(taos->proximity_input_dev);
	for (i = 0; i < 4; i++) {
...
int taos_get_lux(void)
{
	...
	{
		pr_err("%s: Light sensor had been off return 0\n", __func__);
		return 0;
	}
	...
	if (cleardata >= 9000 || irdata >= 9000) {
		...
		if (proximity_value == 1) {
			...
			if(USE_INPUT_DEVICE) {
				input_report_abs(taos->proximity_input_dev,ABS_DISTANCE,!proximity_value);
				pr_err("%s: saturation. value(%d)(0: close, 1: far) \n", __func__, !proximity_value);
				input_sync(taos->proximity_input_dev);
				usleep_range(1000, 2000);
			}
		}
...
static void taos_chip_on(void)
{
	...
	if (fail_num == 0) {			
		...
		pr_err("%s: I2C failed in taos_chip_on, # of fail I2C=[%d]\n", __func__, fail_num);
	}
}    
...
static int taos_chip_off(void)
{
	...
	if (err) 
		pr_err("%s: disable wakeup source failed\n", __func__);
#endif
...
{
	pr_err("%s : type=%d (0:light, 1:prox, 2:all)\n", __func__, type);
	taos_chip_on();
	if (type == TAOS_PROXIMITY || type == TAOS_ALL) {
		enable_irq(taos ->irq);
		pr_err("enable irq for proximity\n");
		proximity_enable = 1;
	...
	if(type == TAOS_LIGHT || type == TAOS_ALL) {
		...
		light_polling_time = ktime_add_us(light_polling_time,200000);
		hrtimer_start(&taos->timer,light_polling_time,HRTIMER_MODE_REL);
		pr_err("%s: lightsensor polling, delay %lldns\n",
...
void taos_off(struct taos_data *taos, int type)
{
	...
	if (type == TAOS_PROXIMITY || type==TAOS_ALL) {
		...
		pr_err("%s: disable irq for proximity \n", __func__);
		proximity_enable = 0;
    	...
    	if(type ==TAOS_LIGHT || type==TAOS_ALL) {
		hrtimer_cancel(&taos->timer);
		pr_err("%s: timer cancel for light sensor\n", __func__);
		light_enable = 0;
