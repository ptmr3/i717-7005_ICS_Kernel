...
//kk0704.park :: INTEGRATION FROM ICS_MAIN	usleep_range(high_period * USEC_PER_MSEC, high_period * USEC_PER_MSEC);
	mdelay(high_period);	//kk0704.park :: INTEGRATION FROM ICS_MAIN

	ret = mhl_tx_clear_reg(sii9234, MHL_TX_DISC_CTRL7_REG, (1<<7) | (1<<6));
	if (ret < 0)
		return ret;

//kk0704.park :: INTEGRATION FROM ICS_MAIN	usleep_range(low_period * USEC_PER_MSEC, low_period * USEC_PER_MSEC);
	mdelay(low_period);	//kk0704.park :: INTEGRATION FROM ICS_MAIN
...
static int sii9234_cbus_reset(struct sii9234_data *sii9234)
{
	...
	mdelay(T_SRC_CBUS_DEGLITCH);	//kk0704.park :: msleep -> mdelay
...
{
	//printk("@@HAPPYROOT@@  %s \n", __func__);
	mdelay(T_SRC_CBUS_FLOAT);	//kk0704.park :: msleep -> mdelay
...
//kk0704.park :: usleep -> mdelay	usleep_range(3000, 4000);
	mdelay(4); //kk0704.park//kk0704.park :: usleep -> mdelay
...
static void cbus_handle_set_int_recd(struct sii9234_data *sii9234)
{
	...
	if (intr_reg1 & MHL_INT_EDID_CHG) {
		 ...
		 */
		mdelay(T_HPD_WIDTH);	//kk0704.park :: msleep -> mdelay
...
int rsen_state_timer_out(struct sii9234_data *sii9234)
{
	...
	if (value & RSEN_STATUS) {
		...
		mdelay(T_SRC_RXSENSE_DEGLITCH);	//kk0704.park :: msleep -> mdelay
		ret = mhl_tx_read_reg(sii9234, MHL_TX_SYSSTAT_REG, &value);
		if (ret < 0)
			goto err_exit;
...
static void goto_d3(void)
{
	...
	mdelay(25);    //kk0704.park :: msleep -> mdelay
...
unhandled:
	...
	if (sii9234->state == STATE_DISCONNECTED) {
		pr_cont(" (timeout)");
		mutex_unlock(&sii9234->lock);
#if 1 //kk0704.park :: ndef CONFIG_MHL_D3_SUPPORT
		mhl_onoff_ex(0);
#endif
		return handled;
...
//kk0704.park			usleep_range(T_SRC_VBUS_CBUS_TO_STABLE * USEC_PER_MSEC,
//kk0704.park				T_SRC_VBUS_CBUS_TO_STABLE  * USEC_PER_MSEC);
			mdelay(280);
...
#endif
			mdelay(T_SRC_RXSENSE_DEGLITCH);	//kk0704.park :: msleep
